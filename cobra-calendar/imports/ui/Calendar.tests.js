...
import { militaryTimeForIndex, slotsForRawEvents } from './Calendar';
...
if (Meteor.isClient) {
  ...
  describe('slotsForRawEvents()', function () {
    it('events starting at 8:00AM go in the first slot', function () {
      const rawEvent = { color: 'green', start: new Date(2020, 0, 1, 8, 0), end: new Date(2020, 0, 1, 8, 15), description: 'desc' };
    ...
    it('events starting at 8:45PM go in the last slot', function () {
      const rawEvent = { color: 'green', start: new Date(2020, 0, 1, 20, 45), end: new Date(2020, 0, 1, 21, 0), description: 'desc' };
      const slots = slotsForRawEvents([rawEvent], 4);
    ...
    it('events spanning multiple hours give slots for multiple hours', function () {
      const rawEvent = { color: 'green', start: new Date(2020, 0, 1, 12, 45), end: new Date(2020, 0, 1, 14, 45), description: 'desc' };
      const slots = slotsForRawEvents([rawEvent], 4);
    ...
    it('events with times not in increments of 15 minutes extend to 15 minute slots', function () {
      ...
      const slots = slotsForRawEvents([rawEvent], 4);
      assert.equal(_.isEqual(slots[1245], {start: 1245, color: 'green', description: 'desc'}), true);
      assert.equal(_.isEqual(slots[1415], {start: 1415, color: 'green', description: 'desc'}), true);
    });
  });
}
